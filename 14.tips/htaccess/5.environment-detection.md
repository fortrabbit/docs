...
You can extend the concept to have more stages of course (see our [multi staging article](/14.tips/staging-environments.md)). To add custom rules for a staging environment, like http-auth (see [article](/14.tips/htaccess/3.http-auth.md)), create a matching `.htaccess_staging` file and set the `ENVIRONMENT` accordingly.
