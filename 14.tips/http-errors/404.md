...
Maybe your software is using a different root path than the one that is currently set? Check the root path settings `htdocs` is the default root path if no specific software has been chosen in the [software preset](/11.concepts/software-presets.md). Now, if you decide to install another software later on, you might have to set the root path accordingly. Best upload all files into `htdocs` directly, not into an extra folder that contains the files.
