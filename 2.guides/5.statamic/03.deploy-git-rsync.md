...
description: 'Deploy Statamic on fortrabbit with git for the code and rsync for user-generated content, which keeps version control clean.'
naviTitle: Git + rsync deployment
navigation.excerpt: Code is version controlled, content is synced
lead: Deploy Statamic on fortrabbit using Git for code and rsync for user-generated content, keeping your version control clean.
figure:
  emoji: 🔁
  hide: true
subject: statamic
...
To connect your GitHub repo with an app on fortrabbit, you first need to install the [fortrabbit GitHub app](/1.platform/06.deployment/02.github-app.md). In many cases, this done during boarding. See your account settings if GitHub is connected:
...
:BlockLink{title="Connect app to GitHub repo" path="/change/git-settings"}
...
Choosing Statamic as the software applies the [software template](/1.platform/03.concepts/05.software-templates.md), which sets the root path, the build and post-deploy commands and the ENV vars an app needs to reach its database.

:SoftwarePreset{slug="statamic"}
