...
description: 'The recommended Kirby workflow on fortrabbit: git carries the code, rsync carries the content written through the panel.'
naviTitle: Git + rsync deployment
navigation.excerpt: Git for code, rsync for content
lead: This is the recommended workflow to deploy Kirby using Git. It's a bit more advanced, but worth the while when maintaining a project.
figure:
  emoji: 🔗
...
head:
  meta:
    - name: keywords
      content: 'Kirby deployment, Git rsync, content synchronization, GitHub integration, fortrabbit'
topics:
  - kirby
  - cms
  - php
...
- Install the [fortrabbit GitHub app](/1.platform/06.deployment/02.github-app.md) at GitHub.
...
Initialize a local Git repository and connect it to GitHub to enable automatic deployment on fortrabbit. See the [git deployment intro](/1.platform/06.deployment/01.intro.md) to get an idea how to connect a local repo to the GitHub remote and enable automatic deployment by installing the [fortrabbit GitHub App](/1.platform/06.deployment/02.github-app.md) with GitHub account.
...
You can also use [SFTP](/1.platform/05.code-access/04.sftp.md) to synchronize the `content` folder.
