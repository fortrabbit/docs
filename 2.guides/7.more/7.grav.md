...
We usually advice to run [your project locally](/3.dev/20.how-to/1.local-development.md) and use [Git deployment](/3.dev/03.deployment/01.intro.md). Since Grav CMS is a static file content management system where files may change on the environment, we can not advice to use git deployment, since it is designed to work as a one way street.
