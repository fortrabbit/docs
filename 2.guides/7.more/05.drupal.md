...
It's assumed you've already created a new app and chose Drupal in the [software template](/1.platform/03.concepts/05.software-templates.md). If not: You can do so in the fortrabbit dashboard. You should have a [local PHP development environment](/3.dev/20.how-to/1.local-development.md) running on your local machine. Drupal has good backward compatibility and this guide should work for Drupal 9, 10, and 11. We recommend following the [official Drupal installation guide](https://www.drupal.org/docs/installing-drupal) to install Drupal locally first. You can start with Composer like so:
...
If you have chosen Drupal when creating the environment in the dashboard, 'web' was pre-selected as the root path. If you have a different root path, go to the dashboard and set the [root path](/1.platform/06.settings/12.root-path.md) to `web`.

:BlockLink{title="Change the root path for {{app-env-id}}" path="/environments/{{app-env-id}}/settings/rootpath"}

## MySQL configuration

If you have chosen Drupal in the [software preset](/1.platform/03.concepts/05.software-templates.md) when creating your app, we will automatically populate the required environment variables for the MySQL connection. The database configuration uses environment variables which are automatically set by fortrabbit.
...
For your local development setup, you can create a `settings.local.php` file with your local database credentials. See the [ENV var topic](/3.dev/19.env-vars/01.intro.md) for more details.

## Deployment

Drupal supports Git-driven applications and Composer-driven workflows perfectly. Deploy via GitHub using our [GitHub integration](/1.platform/05.deployment/02.github-app.md). SSH and SFTP deployment methods are also available.
...
Your `settings.php` file should be configured to work with fortrabbit's environment. Here's a recommended configuration:
...
// Database configuration (handled by fortrabbit environment variables)
$databases['default']['default'] = [
  'database' => $_ENV['MYSQL_DATABASE'],
  'username' => $_ENV['MYSQL_USER'],
  'password' => $_ENV['MYSQL_PASSWORD'],
...
$settings['trusted_host_patterns'] = [
  '^' . preg_quote($_ENV['APP_URL_HOST'] ?? 'localhost') . '$',
];

// Hash salt
$settings['hash_salt'] = $_ENV['DRUPAL_HASH_SALT'] ?? 'your-hash-salt-here';
...
Set these environment variables in your fortrabbit dashboard:
