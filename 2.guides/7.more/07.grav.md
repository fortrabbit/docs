...
We usually advice to run [your project locally](/4.integrations/01.local-development/01.intro.md) and use [Git deployment](/1.platform/05.deployment/01.intro.md). Since Grav CMS is a static file content management system where files may change on the environment, we can not advice to use git deployment, since it is designed to work as a one way street.
