...
We usually advise running [your project locally](/4.integrations/01.local-development/01.intro.md) and using [Git deployment](/1.platform/05.deployment/01.intro.md). Since Grav CMS is a file-based content management system where files may change on the environment, we cannot advise using Git deployment, since it is designed to work as a one-way street.
...
2. Copy all **contents** of the local `grav-admin` folder (not the folder itself) via [SFTP](/1.platform/04.code-access/04.sftp.md) to the `htdocs` folder of your environment.
