---
title: Tempest guide
description: 'Tempest is a PHP framework built on automatic code discovery, with no config files for routes or commands. It needs PHP 8.5.'
reviewed: 2026-07-06
reviewer: fl
naviTitle: Tempest
...
**Optional but recommended**: To enable [git deployment](/1.platform/06.deployment/01.intro.md), init git and add GitHub as a remote repo, either as public or private. Here's how to use [GitHub CLI](/4.integrations/07.git-clients/02.gh-cli.md) to set up a local repo and connect it to GitHub:
...
You can also use [SSH](/1.platform/05.code-access/03.ssh.md) / [SFTP](/1.platform/05.code-access/04.sftp.md) or [rsync](/3.dev/20.how-to/rsync.md) to deploy, or integrate with your existing pipelines using deploy keys.
...
If not already, set the [root path](/1.platform/07.settings/12.root-path.md) to `public` in the dashboard.
