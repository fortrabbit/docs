...
reviewer: fl
title: Symfony guide
description: 'Install and tune Symfony on fortrabbit: the local installation, git deployment, and the production settings the framework expects.'
naviTitle: Symfony
...
head:
  meta:
    - name: keywords
      content: 'symfony, php, fortrabbit, composer, mysql, deployment'
topics:
  - frameworks
  - php
---
...
Follow the [official Symfony guides](https://symfony.com/doc/current/setup.html) to create a local installation of Symfony if you don't already have one.
...
- [Deployment intro](/1.platform/06.deployment/01.intro.md) - concepts on fortrabbit
...
If you did not choose Symfony when creating the environment in the dashboard at first, please set the following: Go to the dashboard and set the [root path](/1.platform/07.settings/12.root-path.md) to 'public'.
...
See the [GitHub app](/1.platform/06.deployment/02.github-app.md) guide on how to connect fortrabbit to your Git repo.

## MySQL

The database should be configured out of the box. See [MySQL access](/1.platform/08.mysql/01.access/01.intro.md) for connection details and configuration.
...
You may want to run migrations and maybe load fixtures on the instance. You can login via [ssh](/1.platform/05.code-access/03.ssh.md) in to your environment, or instead just fire single commands like so:
