...
Another reason that assets should be managed and deployed independently from Git deployment, is that files uploaded to the environment would not be represented in Git, as [Git deployment is a one way street](/1.platform/05.deployment/01.intro.md#git-only-syncs-up).
...
Good old [SFTP access](/1.platform/04.code-access/04.sftp.md) is another valid way to do it. Just fire up your SFTP client, log in to the App and manage the assets manually. Many SFTP clients are offering sync options, to keep local and remote files up-to-date.
