...
The `config/general.php` file we use in [our setup guide](/2.guides/3.craft-cms/01.install.md) sets `allowUpdates` to `false`: this prevents plugin and Craft core updates from the Craft Control Panel in the `production` app environment.
...
During the [Git deployment](/3.dev/03.deployment/3.build-commands.md) `composer install` will likely run automatically. This way your local Composer changes get applied on the remote.
...
When you have chosen Craft CMS in the [software preset](/1.platform/02.concepts/05.software-templates.md) while creating the app environment, the following two commands are part of your [deployment build steps](/3.dev/03.deployment/3.build-commands.md).
