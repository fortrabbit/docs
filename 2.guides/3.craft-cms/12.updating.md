...
description: 'Keep Craft CMS current for security: update the local installation first, test it there, then deploy that state to fortrabbit.'
naviTitle: Updating
navigation.excerpt: Guide to upgrading Craft CMS
...
head:
  meta:
    - name: keywords
      content: 'craft CMS update, craft CMS upgrade, database migration, composer, fortrabbit'
topics:
  - craft-cms
  - cms
  - php
...
Your [local development environment](/4.integrations/01.local-development/01.intro.md) is where changes are applied and tested first. Please update your local Craft CMS installation first and apply updates by deploying as a second step. That way you make sure that your environments are in sync.

The `config/general.php` file we use in [our setup guide](/2.guides/3.craft-cms/01.install.md) sets `allowUpdates` to `false`: this prevents plugin and Craft core updates from the Craft Control Panel in the `production` environment. Keep your environments in sync and always update locally first; avoid updates and plugin installations directly on the remote environment.

Here are the options to update Craft CMS and Craft plugins. Also see the [official Craft CMS update documentation](https://craftcms.com/docs/5.x/update.html).
...
During the [Git deployment](/1.platform/06.deployment/03.build-commands.md) `composer install` will likely run automatically. This way your local Composer changes get applied on the remote.
...
When you have chosen Craft CMS in the [software preset](/1.platform/03.concepts/05.software-templates.md) while creating the environment, the following two commands are part of your [deployment build steps](/1.platform/06.deployment/03.build-commands.md).
...
You can trigger the changes via the Craft CLI in the terminal via [SSH](/1.platform/05.code-access/03.ssh.md):
