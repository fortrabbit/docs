...
figure:
  emoji: ⬆️
  text: Update local first, then deploy.
  size: sm
sidebar: craft-cms
nextNav: true
head:
...
The `config/general.php` file we use in [our setup guide](/2.guides/3.craft-cms/01.install.md) sets `allowUpdates` to `false`: this prevents plugin and Craft core updates from the Craft Control Panel in the `production` environment.
