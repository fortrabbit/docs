...
During the [Git deployment](/1.platform/05.deployment/03.build-commands.md) `composer install` will likely run automatically. This way your local Composer changes get applied on the remote.
...
When you have chosen Craft CMS in the [software preset](/1.platform/03.concepts/05.software-templates.md) while creating the environment, the following two commands are part of your [deployment build steps](/1.platform/05.deployment/03.build-commands.md).
...
You can trigger the changes via the Craft CLI in the terminal via [SSH](/1.platform/04.code-access/03.ssh.md):
