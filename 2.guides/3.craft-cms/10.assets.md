...
Another reason that assets should be managed and deployed independently from Git deployment, is that files uploaded to the app environment would not be represented in Git, as [Git deployment is a one way street](/3.dev/03.deployment/01.intro.md#git-works-only-one-way).
...
Good old [SFTP access](/3.dev/01.code-access/4.sftp.md) is another valid way to to do it. Just fire up your SFTP client, log in to the App and manage the assets manually. Many SFTP clients are offering sync options, to keep local and remote files up-to-date.
