...
Another reason that assets should be managed and deployed independently from Git deployment, is that files uploaded to the environment would not be represented in Git, as [Git deployment is a one way street](/3.dev/03.deployment/01.intro.md#git-works-only-one-way).
