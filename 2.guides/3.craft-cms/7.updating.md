...
Your [local development environment](/3.dev/1.local-development.md) is where changes are applied and tested first. Please update your local Craft CMS installation first and apply updates by deploying as a second step. That way you make sure that your environments are in sync.
...
When you have chosen Craft CMS in the [software preset](/1.platform/11.concepts/software-templates.md) while creating the app environment, the following two commands are part of your [deployment build steps](/6.deployment/3.build-commands.md).
...
You can trigger the changes via the Craft CLI in the terminal via [SSH](/3.dev/7.code-access/3.ssh.md):
