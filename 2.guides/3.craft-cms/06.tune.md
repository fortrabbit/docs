...
description: 'Run Craft CMS well on fortrabbit: environment variable configuration, dev mode, HTTPS, caching and the settings worth changing.'
naviTitle: Tuning
...
head:
  meta:
    - name: keywords
      content: 'craft cms, tuning, configuration, environment variables, production, fortrabbit'
topics:
  - craft-cms
  - cms
  - php
...
Craft CMS uses modern `.env` style configuration, see [topic ENV vars](/3.dev/19.env-vars/01.intro.md). This means you can run your Craft locally and remotely without code or configuration file changes. Locally, your `.env` file will be modified and read. That file is not part of Git, you don't need it on fortrabbit.

### Manually setting ENV vars

The [software templates](/1.platform/03.concepts/05.software-templates.md) will populate required ENV vars. If you didn't choose the Craft preset when creating the app or the ENV vars have been deleted, this is likely what needs to be set.
...
The syntax of the keys may differ by Craft CMS version. See [ENV var usage](/3.dev/19.env-vars/01.intro.md) for more details on aliases.

## Software template

If Craft CMS is detected or you have chosen it in the software list, the Craft CMS [software template](/1.platform/03.concepts/05.software-templates.md) will be applied. This will pre-configure common ENV vars, build and post-deploy commands and set the root path to `web`.

## Database setup

On fortrabbit the [environment variables](/3.dev/19.env-vars/01.intro.md) are seeded from the ones set in the fortrabbit dashboard (not from the .env file). If you chose Craft in the [software preset](/1.platform/03.concepts/05.software-templates.md) when creating the app, all ENV vars at fortrabbit will already be pre-populated.
...
Craft CMS comes with a predefined `.htaccess` file that lives inside the `web` folder. You can extend that with your own rules, like forwarding all requests to https or disabling access on the App URL. Please see our [.htaccess articles](/3.dev/21.htaccess/01.intro.md) for examples.
...
Sometimes while developing you might want to see some error output directly on your browser screen. That's what the `devMode` flag is for. See the [Craft docs](https://craftcms.com/knowledge-base/what-dev-mode-does) for more details.

DevMode is disabled by default via [software template](/1.platform/03.concepts/05.software-templates.md) on fortrabbit as the installation is public. You can change that setting with [ENV vars](/1.platform/07.settings/01.env-vars.md) in the fortrabbit dashboard:
...
Craft CMS comes with a built in command line interface which can be called from the console. You can also run it on the environment itself like so:
...
The Craft CMS stream log location can be set to `stderr` and `stdout` by setting the [environment variable](/3.dev/19.env-vars/01.intro.md) `CRAFT_STREAM_LOG=true`. This is pre-populated when choosing Craft CMS in the [software chooser](/1.platform/03.concepts/05.software-templates.md) while creating the app. When the ENV var is present and true, Craft CMS error logs can be accessed through the fortrabbit dashboard.
...
See our [quirks article](/1.platform/03.concepts/09.limits.md) on limits sending emails. Also see [transactional mail provider integration](/4.integrations/10.transactional-mail/01.intro.md).
