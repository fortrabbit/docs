...
Craft CMS uses modern `.env` style configuration, see [topic ENV vars](/3.dev/19.env-vars/1.intro.md). This means you can run your Craft locally and remotely without code or configuration file changes. Locally, your `.env` file will be modified and read. That file is not part of Git, you don't need it on fortrabbit.

### Manually setting ENV vars

The [software templates](/1.platform/02.concepts/05.software-templates.md) will populate required ENV vars. If you didn't choose the Craft preset when creating the app or the ENV vars have been deleted, this is likely what needs to be set.
...
The syntax of the keys may differ by Craft CMS version. See [ENV var usage](/3.dev/19.env-vars/1.intro.md/2.usage.md) for more details on aliases.

## Software template

If Craft CMS is detected or you have chosen it in the software list, the Craft CMS [software template](/1.platform/02.concepts/05.software-templates.md) will be applied. This will pre-configure common ENV vars, build and post-deploy commands and set the root path to `web`.
...
Craft CMS comes with a predefined `.htaccess` file that lives inside the `web` folder. You can extend that with your own rules, like forwarding all requests to https or disabling access on the App URL. Please see our [.htaccess articles](/3.dev/6.htaccess/index.md) for examples.
...
The Craft CMS stream log location can be set to `stderr` and `stdout` by setting the [environment variable](/3.dev/19.env-vars/1.intro.md) `CRAFT_STREAM_LOG=true`. This is pre-populated when choosing Craft CMS in the [software chooser](/1.platform/02.concepts/05.software-templates.md) while creating the app. When the ENV var is present and true, Craft CMS error logs can be accessed through the fortrabbit dashboard.

## Sending mail

You can not use sendmail on fortrabbit - see our [quirks article](/1.platform/02.concepts/04.limits.md). Craft CMS allows your to configure sending mail via SMTP. It includes options to set your SMTP host, port, and credentials. You can use environment variables there as well. It's also possible to do that configuration within the Craft control panel. Our recommendation: Use an external third party transactional mail provider for that. Pixel & Tonic (Craft CMS creators) maintain a [plugin for Postmark](https://plugins.craftcms.com/postmark). With that plugin installed you can easily set it up.
