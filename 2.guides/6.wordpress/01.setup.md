---
rank: 5
reviewed: 2026-07-04
reviewer: fl
title: WordPress quick setup guide
description: 'Install WordPress on a fortrabbit environment over SSH. Git deployment is not recommended here, because WordPress writes to its own files.'
naviTitle: Setup
navigation.excerpt: Get WordPress up and running quickly
lead: Download and install WordPress on fortrabbit. Use SSH to deploy WordPress files directly to your environment.
figure:
  emoji: 🧩
  hide: true
subject: wordpress
head:
  meta:
    - name: 'keywords'
      content: 'WordPress, install, setup, CMS, blogging, fortrabbit'
topics:
  - wordpress
  - cms
  - php
---

We assume you've already created an [app](/1.platform/10.objects/01.app.md) and chose WordPress in the [software template](/1.platform/03.concepts/05.software-templates.md). In the following, you will install WordPress directly on your [environment](/1.platform/10.objects/02.environment.md) via SSH. We do not recommend using [git deployment](/1.platform/06.deployment/01.intro.md) for WordPress, as it requires dynamic file uploads and database configuration that are not suited to code-only deployment workflows.

## Software template

WordPress carries no settings of its own, so an app running it deploys with the platform defaults:

:SoftwarePreset{slug="wordpress"}

## Install WordPress

To install WordPress, log in to your environment via SSH and download the latest version directly from the official WordPress repository. Run the following commands in sequence to extract and deploy WordPress to your app:

```shell
# 1. Login to your environment via SSH
$ ssh {{app-env-id}}@ssh.{{region}}.frbit.app

# Once you are logged in:

# 2. Download latest WordPress
$ wget https://wordpress.org/latest.tar.gz

# 3. Unpack wordpress
$ tar zxf latest.tar.gz

# 4. Move unpacked files to top level
$ mv wordpress/* .

# 5. Remove the downloaded container
$ rm -r wordpress latest.tar.gz
```

## Run the installer

Once the files are in place, visit your environment's test domain to run the installer:

:BlockLink{title="Visit test domain" path="https://{{app-env-id}}.frbit.app" property="none"}

Complete the guided web installation to finish the setup. The WordPress installer will ask you for database credentials, which you can find in the dashboard under your environment settings, along with your site's title, admin username, password, and email. After the guided setup completes, you will be redirected to the WordPress admin login form where you can use your chosen credentials to log in.

## Installing themes and plugins

After WordPress is running, you can manage themes and plugins through the WordPress admin dashboard. You can download and install themes and plugins directly from the WordPress marketplace within wp-admin, or upload custom themes and plugins to your app's `/wp-content/` folder via SFTP or SSH.
