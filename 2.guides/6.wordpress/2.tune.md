...
Your environment URL `{{app-env-id}}.frbit.app` is the first address your WordPress can be reached from. Later on, when you go live, you will add your own custom external domains. Here is the basic setup:

1. Register and route the domain to the fortrabbit App < see [domain article](/1.platform/10.objects/10.domain.md)
...
WordPress consists of code files, the user generated uploads and of course the [MySQL database](/1.platform/9.components/2.database.md), in which most of the contents are stored. There are various use cases to export and import the database:
...
Sophisticated developers might have a look at rsync. It's many times faster than SFTP. See our [rsync article](/3.dev/rsync.md).
