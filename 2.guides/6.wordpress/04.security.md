...
You cannot use [sendmail](/1.platform/02.concepts/4.quirks.md#mailing) on fortrabbit but you can use a SMTP plugin like [WP SMTP](http://wordpress.org/plugins/wp-smtp) or [MAIL SMTP](http://wordpress.org/plugins/wp-mail-smtp) to enable SMTP support for the `wp_mail()` function.
