...
Only the branches `main`, `master` and the `{{app-env-name}}` will be deployed to the webspace. More [here](../2.git.md#the-branch-name-counts).

### Release package limit

To keep deployment fast for everyone the size of the release package is limited to 200 MB. [Read on](../9.platform/3.environment/git.md#release-package-limit).
