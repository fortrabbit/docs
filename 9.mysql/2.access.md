...
* **Database Username**: `{{app-env-name}}`
...
If possible, we recommend to use [MySQL env vars](/13.platform/4.details/env-vars.md) instead of hard coding them into your configuration files. The automatically available environment variables are:
...
$ mysql -u{{app-env-name}} -h127.0.0.1 -P13306 -p -D {{app-env-name}}
```
...
```
$cfg['Servers'][$i]['verbose']       = '{{app-env-name}}';
...
Then open a [terminal tunnel](#mysql-via-terminal), then visit your local phpMyAdmin in the browser. You now can select your fortrabbit App. You will be asked for the MySQL user "**{{app-env-name}}**" and [password](#obtain-the-mysql-password). Using a local phpMyAdmin with your remote database requires you to always open a tunnel first - a [MySQL GUI](#mysql-via-gui) might be the better choice.
