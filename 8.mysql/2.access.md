...
That is only a generic example. See our specific guides for: [Laravel](install-laravel-uni#mysql), [Symfony](install-symfony-6#mysql), [WordPress](install-wordpress-uni#mysql), [Craft CMS](craft-tune) which are often using zero-config style environment variables.
