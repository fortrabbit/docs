...
excerpt:       Debugging SSH/SFTP connection issues
...
```bash
ssh {{app-env-name}}@deploy.{{region}}.frbit.com -v
```

### Access error after an environment was created

If an app / environment was recently created, please wait about 5 minutes for it to become ready. Trying to connect (ssh, sftp) before it is ready produces an error.

If more than 10 minutes have passed and you still get a similar error (despite successful authentication), then please contact us.

<!--  TODO: Merge this below   -->

## Read first please

Please make sure to understand the available [deployment methods](/deployment-methods) and [code access methods](/access-methods) on fortrabbit. Make also sure to read [SSH key setup guide](8.ssh-keys.md).
...
```bash
ssh -i ~/.ssh/id_rsa_fortrabbit {{app-env-name}}@deploy.{{region}}.frbit.com
```
...
Read this: [remove the public keys from your fortrabbit Account](access-methods#how-to-change-from-ssh-key-to-password-authentication)
