...
Using our [Git deployment](2.git.md) is great, but sometimes need a little more control. That's where the Secure SHell comes in. SSH is the big brother of [SFTP](sftp) and allows you to directly interact with your App's code. You may want to set up an [SSH Key](8.ssh-keys.md) if you don't have one already.
...
Also checkout: [WordPress install from SSH](install-wordpress-4-uni#installing-wordpress-with-ssh), [Execute Laravel's artisan](install-laravel-5-uni#migrate-amp-other-artisan-commands).
...
- [exporting and importing a MySQL dump](https://help.fortrabbit.com/mysql-export-import#mysql-export-import-on-the-command-line)
- [using the standard mysql client](https://help.fortrabbit.com/mysql#mysql-via-terminal)
