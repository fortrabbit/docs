...
excerpt:       Connect your git repo to deploy for automated deployment
...
We assume that you have: [Git installed](3.guides/1.craft/more/git.md) locally and know the basics. We further assume that you know about [access methods](/access-methods) here on fortrabbit, so either have your SSH keys installed or your [Dashboard](/dashboard) password handy.
...
[Composer](composer.md) install will always run after a successful `git push` - as long as both `composer.json` and `composer.lock` are present. Read on [here](composer.md).
