...
We assume you've already created an [App](app) and chose WordPress in the [Software Preset](app#software-preset). If not: You can do so in the [fortrabbit Dashboard](/dashboard). Following the fastest way to start with a fresh installation. Please scroll below for [migrating an existing WordPress](#advanced-setup).
