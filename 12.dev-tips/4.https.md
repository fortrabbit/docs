...
That's the most easiest and most obvious way to quickly run your App on https. You can already access your [App URL](app#app-url) using HTTPS. For example: `https://{{app-env-name}}.frb.io`. Use this URL for testing or if a custom domain is not important to you.
...
It is recommended to forward all requests to the secure line, so no more "http://", only "http**s**://". See [an example here](/htaccess#redirect-all-requests-to-https).
