...
Your [local development environment](/15.tips/1.local-development.md) is where changes are applied and tested first. Please update your local Craft CMS installation first and apply updates by deploying as a second step. That way you make sure that your environments are in sync.
...
During the [Git deployment](/6.deployment/3.build-commands.md) `composer install` will likely run automatically. This way your local Composer changes get applied on the remote.
...
When you have chosen Craft CMS in the [software preset](/11.concepts/software-templates.md) while creating the app environment, the following two commands are part of your [deployment build steps](/6.deployment/3.build-commands.md).
