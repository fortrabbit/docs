...
This can be a misunderstanding: You might have thought that when you choose a [Software Preset](//12.platform/software-presets.md) software will be installed. Sorry, no one-click-installs here, you need to install the software yourself. So please go ahead and deploy some code first.
...
Maybe your software is using a different root path than the one that is currently set? Check the [root path](/app#root-path) settings.

`htdocs` is the default [root path](/app#root-path) if no specific software has been chosen in the [Software Preset](#software-preset). Now, if you decide to install [Laravel](install-laravel) or any other software later on, you might have to set the root path accordingly.
