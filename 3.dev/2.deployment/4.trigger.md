...
When manual deployment is selected, no automatic deployments will happen. Initiate a deployment from the [dashboard](/1.platform/11.concepts/dashboard.md).
...
:DashboardLink{title="Deploy {{app-env-id}} now" path="/confirm/environment-deploy?environment={{app-env-id}}"}
