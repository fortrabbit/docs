...
[Domains](/1.platform/10.objects/10.domain.md) registered with the [dashboard](/1.platform/02.concepts/30.dashboard.md) have a target setting. You can choose to route the domain directly to the root path of an environment or to be forwarded.
...
When adding a www domain to the dashboard you will be provided a CNAME record (host name) for the www prefix and another domain for the apex domain. Here the routing is via A record (IP address). The IP address is the forwarding service. You will add both records with your [domain provider](/3.dev/13.dns/10.domain-providers.md).
...
- [ALIAS / ANAME routing](/3.dev/13.dns/aname-alias-records.md)
- Forwarding, using your domain provider
- Use `.htaccess` for more fine tuned control, see [htaccess redirects](/3.dev/htaccess/2.redirects.md)
