...
Development is usually happening in the [local development environment](/15.tips/1.local-development.md). Code changes are pushed up.
...
In addition to git deployment, [direct code access](/7.code-access/1.intro.md) via [SSH](/7.code-access/3.ssh.md) and [SFTP](/7.code-access/4.sftp.md) is also available. This is useful to sync down contents.
