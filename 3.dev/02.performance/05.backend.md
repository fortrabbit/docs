...
- APC: Aside from opcode caching which happens automatically, APC works as an object cache, similar to Memcache or Redis.
...
- **Shrink your loader chain**: Make sure you load only the classes (files) you need. For a production App (in which code changes do not occur often) you can set `apc.stat` or `opcache.validate_timestamps` to `0`, which reduces I/O on `.php` files even further, but requires an APC/OPcache flush for changed files to apply.
