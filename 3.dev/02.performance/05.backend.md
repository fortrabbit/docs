...
description: 'Backend performance on fortrabbit: time to first byte, caching layers, and designing for scale before the traffic arrives.'
...
head:
  meta:
    - name: keywords
      content: 'backend performance, PHP performance, caching, database optimization, load testing, fortrabbit'
topics:
  - performance
  - php
---

We want to encourage you to **design for performance** and scalability. And that is all about thinking a bit ahead. Speed matters for your users and for SEO. You - the responsible PHP developer - may have more responsibility than you think.

## Time to first byte

The TTFB depends on how fast the service can compute. Your code plays a major role. Also see the [PHP processes article](/3.dev/02.performance/06.php-processes.md) to learn about limits when executing PHP. See our [understand performance article](/3.dev/02.performance/04.monitoring.md) on how to analyze backend performance problems.

### Prepare to cache

Implement caching early and behind an abstraction, so the store can be swapped later without touching the calling code. Available caching solutions on fortrabbit:

- File: Disk storage is not very fast.
- Database: Faster than disk. Use if you need to cache data for a long time.
- OPcache: Opcode caching happens automatically. It caches compiled PHP, not application data, so it is not a store for cached values.
...
- **Avert files**: Replace file-sessions, file-caches and anything `file-*` with the database where the access pattern allows it.
...
- **Shrink your loader chain**: Make sure you load only the classes (files) you need. For a production App (in which code changes do not occur often) setting `opcache.validate_timestamps` to `0` reduces I/O on `.php` files even further, but changed files then need an OPcache flush to apply.
...
If you're planning on growing large and expect a lot of traffic, you will end up putting your static files (images, videos, css and js files, ...) behind a Content Delivery Network. Nowadays, you can do this fairly easily with specialized services. Some services work best with a dedicated (sub)domain for your static contents, such as _img.yourdomain.com_. If you expect to use them in the future, you should implement the required structure from the start. Also a good precaution is to setup your cache headers properly from the start.

### Database considerations

Very common: check your JOIN queries! If they ran fast when your dataset was small, it doesn't mean they still do with a bigger dataset. Utilize caches to buffer costly database results. Database optimization could easily fill a whole book — or books. Read our [dedicated MySQL performance tips](/3.dev/02.performance/08.database.md).
...
This is an opinionated topic. If you come from searching 504 time out errors, you will often read to increase the `max_execution_time` setting to allow a certain process to finish its task.
...
Enable our [WAF feature](/1.platform/07.settings/20.firewall-incoming.md).

Set a crawl delay to limit unnecessary crawling of your website if it is not updated very frequently. The crawl-delay directives in `robots.txt` are supported by good bots such as search engines, commercial bots, SEO bots.

## Swap issues

In some cases swap usage by the application is not possible. In these cases you might see dreaded white-pages and accompanying error messages in the log: "Allowed memory size of 1234.. bytes exhausted (tried to allocate 234 bytes)." This should be understood as an urgent reminder to scale up the [PHP plan](/1.platform/09.components/01.php.md).
