...
reviewer: fl
title: PHP processes
description: 'A PHP-FPM process is a reserved slice of computing power. How process count, response time and perceived page speed hang together.'
naviTitle: PHP processes
navigation.excerpt: Deep dive into PHP processes and response time
figure:
  emoji: 🔁
  text: How requests can your app keep busy?
...
head:
  meta:
    - name: keywords
      content: 'PHP processes, response time, PHP memory, FPM, concurrent requests, performance, fortrabbit'
topics:
  - performance
  - php
  - scaling
...
fortrabbit utilizes FPM (FastCGI Process Manager). Unlike with php-cgi the PHP FPM process stays alive. Besides the allocated PHP memory, a number of persistent FPM PHP processes is spawned with each plan. See the [PHP component article](/1.platform/09.components/01.php.md).
...
- Aim for 250 ms or less on average.
  - [Our dashboard shows you PHP response time metrics](/3.dev/02.performance/04.monitoring.md).
...
Page views refer to a single request made by a user to view a page on a website. This request will likely involve a number of HTTP requests, such as images, HTML, CSS, and JavaScript files, all of which must be downloaded from the web server.
