...
description: 'Slow MySQL queries are behind many 504 errors. How to spot them, what indexes fix, and where the database component runs out of power.'
...
lead: 'Website performance problems are often caused by misusage of the MySQL database resources provided. Those not only consist of an available storage size, but also computing power.'
nextNav: true
head:
  meta:
    - name: keywords
      content: 'database performance, MySQL optimization, query optimization, slow queries, database indexing, fortrabbit'
topics:
  - performance
  - mysql
---

Many [504 errors](/3.dev/22.http-status-codes/504.md) and slow PHP response times are related to slow MySQL queries. It's your job as the responsible developer to make sure no insane MySQL queries are executed with your [environment](/1.platform/10.objects/02.environment.md). When using a framework or CMS you might not write MySQL queries directly. Still you need to take that the requests to the database are executed without waste.
...
There are a couple of tools you can use to identify and debug slow queries:
...
APM (Application Performance Monitoring) is a whole industry of its own. In professional environments, deep insights into code performance is crucial. Such tools will also provide stack tracing and issue spotting for database queries. Not all of them are supported with fortrabbit.

- Blackfire - [integration guide](/4.integrations/04.apm/02.blackfire.md)
- New Relic - [integration guide](/4.integrations/04.apm/03.new-relic.md)
- Tideways
- Laravel Nightwatch
- Flare by Spatie
- Datadog
