...
description: 'Hosting headless PHP projects: a Laravel, Symfony, Craft CMS, Kirby or Statamic backend behind an API, with the frontend deployed separately.'
naviTitle: Headless workflows
navigation.excerpt: Working with decoupled applications
figure:
  emoji: 🔗
  text: Headless backend systems with frontend heads.
  color: rgb(20, 184, 166)
  textColor: rgb(224, 251, 251)
...
- The PHP backend - a framework like Laravel, Symfony, or a headless CMS like Craft CMS, Kirby, or Statamic - exposes a RESTful API or GraphQL endpoints.
- The JS frontend - frameworks like Vue.js, React, or Next.js - consumes this data to create dynamic user interfaces, often in the form of Single Page Applications (SPA).
...
It's also possible to have both systems run side by side in one environment at fortrabbit. This has the benefit that the project code base can stay together (maybe in a [single repo](/3.dev/15.learn/04.monorepos.md)) and does not have to be separated over two different hosting systems.
...
HTML is only created in the browser (classic). This is OK for any web application that requires users to login.
...
But what happens, when an editor changes the content of an article? The generation of the pages only happens when the project gets deployed, not on edit. [Deploy hooks](/1.platform/06.deployment/16.deploy-hook.md) (triggered after edit) and incremental builds are aiming to help. But setting that up can be challenging.
...
Headless apps, specifically in SSR mode, can cause considerable server load peaks. Imagine a large website that always needs to be completely rebuilt every time it gets deployed. This involves generating thousands of pages with complicated queries and many images. For the server that means short times of high performance and long periods of idle time.
