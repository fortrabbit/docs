...
reviewer: fl
title: SSH key setup
description: 'Create an SSH key pair and add the public key to a fortrabbit account, for git deployment, SSH sessions and SFTP without a password.'
naviTitle: SSH key setup
navigation.excerpt: Set up SSH keys on your local machine
figure:
  emoji: 🔑
  text: SSH keys intro and setup.
  color: rgb(59, 130, 146)
  textColor: rgb(250, 200, 100)
...
SSH keys are a secure and convenient alternative to passwords. A key pair consists of a public key (shared with the server) and a private key (kept secret). Authentication happens automatically when the server verifies you hold the matching private key. SSH keys are used for [Git deployment](/3.dev/15.learn/git-push-deploy.md) and other secure operations.
...
Proceed to the next section if you don't have any key pairs or get an error about a missing folder. If you do see key pairs like above, you can [add the public SSH key to your account](/1.platform/10.objects/12.ssh-key.md).
...
The procedure to create SSH keys is slightly different on macOS and Linux compared to Windows. There are different ways to set up and use Git with Windows and also different ways to set up and store the keys. We recommend using the official installer from the Git website, together with Git Bash.
