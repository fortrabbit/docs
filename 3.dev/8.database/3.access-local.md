...
```shell
ssh -N -L 13306:{{app-env-id}}.mysql.{{region}}.frbit.app:3306 {{app-env-id}}@tunnel.{{region}}.frbit.app
```
...
- **MySQL Hostname**: `{{app-env-id}}.mysql.{{region}}.frbit.com`
...
`{{app-env-id}}.mysql.{{region}}.frbit.com`.
...
Then open a [terminal tunnel](#mysql-via-terminal), then visit your local phpMyAdmin in the browser. You now can select your fortrabbit App. You will be asked for the MySQL user "**{{app-env-id}}**" and password. Using a local phpMyAdmin with your remote database requires you to always open a tunnel first - a [MySQL GUI](#mysql-via-gui) might be the better choice.
