...
Not having a local development environment can lead to all kinds of problems. Developers break production while performing "open heart surgery"—running major updates directly on the remote server. The fortrabbit platform aims to make deployment from local development to remote environments seamless and safe.
...
Laravel Herd provides a streamlined local development environment specifically optimized for PHP applications on macOS and Windows. It bundles NGNIX, DnsMasq and some other magic to an easy to use statusbar app. It includes a free version, for using MySQL the upgrade must be bought.
