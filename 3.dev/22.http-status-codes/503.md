...
description: 'A 503 means the app crashed or ran out of resources. How to tell an overloaded PHP component from a broken deployment, and what to fix.'
naviTitle: 503 errors
navigation.excerpt: Service unavailable
head:
  meta:
    - name: keywords
      content: 503 error, fortrabbit, service unavailable, HTTP status codes, PHP-FPM, troubleshooting
links:
  - title: Who is in charge for 503?
    route: /dev/performance/people-problems
  - title: Website down help
...
lead: The 503 HTTP status code means "Service Unavailable"—a common error indicating your app has crashed or exhausted its resources.
topics:
  - http
  - troubleshooting
---

- 503 errors will often be shown on screen immediately
- 503 errors often occur after code or configuration changes
- **503 errors also often occur without any changes** - see [here](/3.dev/02.performance/02.people-problems.md)
...
The Apache error log tags this as [`AH01067`](/3.dev/15.learn/apache-errors.md#ah01067-failed-to-read-fastcgi-header). A close relative, [`AH01075`](/3.dev/15.learn/apache-errors.md#ah01075-error-dispatching-request) (*error dispatching request*), points at the same cause — a PHP worker that died mid-request. See [Apache error codes](/3.dev/15.learn/apache-errors.md) for the full list.

## Restarting PHP (workaround)

In some cases, to get the App working again, it may be enough to restart the environment from the Dashboard. If the issue is caused by buggy code, or slow queries, or 3rd party APIs, the problem is likely to happen again.
...
One solution is to upgrade your PHP plan, if possible. Otherwise, you will have to profile the App to figure out why it's using so much memory.
