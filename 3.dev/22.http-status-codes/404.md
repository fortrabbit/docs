...
description: 'A 404 means the server answered but found nothing at that address. The usual causes on fortrabbit — root path, routing and rewrite rules.'
...
lead: A 404 error indicates the requested file isn't found on the server. Learn what causes it and how to resolve it.
head:
  meta:
    - name: keywords
      content: "404 error, file not found, http status code, fortrabbit, troubleshooting, deployment"
topics:
  - http
  - troubleshooting
---
...
You might have an error with the URL. Check for typos in your browser's address bar.
...
The Apache error log names the specific cause with an `AHxxxxx` code. Behind a 404, look for:

- [`AH00128`](/3.dev/15.learn/apache-errors.md#ah00128-file-does-not-exist) — Apache found no file at the requested path under the root path
