...
description: 'A 504 is a gateway timeout: requests take too long and pile up. How to find the slow code path and stop the queue from filling up.'
naviTitle: 504 errors
navigation.excerpt: Gateway timeout
reviewer: fl
links:
  - title: Who is in charge for 504?
...
lead: The 504 HTTP status code means "Gateway Timeout". How to fix slow requests.
head:
  meta:
    - name: 'keywords'
      content: '504 error, HTTP 504, gateway timeout, troubleshoot 504, 503 error, PHP-FPM performance, fortrabbit, server error'
topics:
  - http
  - troubleshooting
---
...
- **504 errors often occur without code changes** - see [Performance problems are often people problems](/3.dev/02.performance/02.people-problems.md)
...
504 and [503 errors](/3.dev/22.http-status-codes/503.md) are two sides of the same coin. PHP-FPM (the PHP process manager here) has too much to do. 504 and 503 errors are often happening simultaneously.
...
- [Performance problems are often people problems](/3.dev/02.performance/02.people-problems.md)
...
Review your code for inefficient queries. If unsure consult a senior colleague or use a profiler like [Blackfire](/4.integrations/04.apm/02.blackfire.md) to find the bottlenecks. Refactor your query design to write better performing MySQL queries or even rethink your data model. Caching certain database results can be another tactic after refactoring.
...
Sometimes bots come visiting your website, scraping each page to feed an AI or a search machine. When not configured correctly, your website may provide such bots infinite numbers of almost identical pages that still need to be rendered and cached.
