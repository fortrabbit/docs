...
description: 'A 408 Request Timeout means the server stopped waiting for the client. Rare on fortrabbit, and usually a slow or unstable connection.'
...
lead: A 408 Request Timeout error means the server stopped waiting for the client to complete sending the request. On fortrabbit, 408 errors are rare and usually indicate a slow client connection rather than a server issue.
head:
  meta:
    - name: keywords
      content: 408, request timeout, HTTP status code, fortrabbit, client connection, timeout errors, troubleshooting
topics:
  - http
  - troubleshooting
---
...
Examine the logs. See [here](/1.platform/07.settings/25.logs.md). The logs may show patterns of when these timeouts occur and help identify the root cause.
