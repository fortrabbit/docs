...
description: 'A 400 Bad Request is rejected before it reaches application code. What triggers it on fortrabbit, often during third-party integrations.'
...
head:
  meta:
    - name: keywords
      content: "400 error, bad request, fortrabbit, http status code, troubleshoot, request validation"
topics:
  - http
  - troubleshooting
---
...
- Application and web server logs will often mention the exact reason. See [Logs](/1.platform/07.settings/25.logs.md) for where to find them.
...
The Apache error log names the specific cause with an `AHxxxxx` code. Behind a 400, look for:

- [`AH00126`](/3.dev/15.learn/apache-errors.md#ah00126-invalid-uri-in-request) — the request line held a malformed URI, usually from a bot or broken proxy
