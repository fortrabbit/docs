...
In some cases we have pre-populated that for you already. See [environment variables topic](/19.env-vars/2.usage.md) for more details on how environment variables are handled here.
...
You can extend the concept to have more stages of course (see our [multi staging article](/15.tips/staging-environments.md)). To add custom rules for a staging environment, like http-auth (see [article](/15.tips/htaccess/3.http-auth.md)), create a matching `.htaccess_staging` file and set the `ENVIRONMENT` accordingly.
