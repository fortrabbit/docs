...
You can always throw more hardware on [performance problems](/3.dev/performance-problems.md). But that won't always help when the problem is in the code somewhere. We want you to encourage to **design for performance** and scalability. And that is all about thinking a bit ahead.

## Backend performance design

This is the part where fortrabbit is involved the most. The TTFB depends on how fast the service can compute. Your code plays a major role. Also see the [PHP processes article](/3.dev/php-processes.md) to learn about limits when executing PHP. See our [performance problems article](/3.dev/performance-problems.md) on how to analyze backed performance problems.
...
Enable our [WAF feature](/1.platform/11.concepts/waf.md).
...
GZIP provides lossless compression for text files such as HTML, CSS or JS. It's implemented on the web server — Apache in our case otherwise nginx — as a module. You can enable and configure it in your `.htaccess` file (see also our [htaccess GZIP article](/3.dev/htaccess/4.gzip.md)).
