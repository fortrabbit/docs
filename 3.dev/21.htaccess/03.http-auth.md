...
When no framework or CMS was chosen when creating the [environment](/1.platform/10.objects/02.environment.md), `htdocs` is the default root folder of the App. Create a new folder, called something like `web` or `root`. Move all your web contents into that folder. Set the newly created folder containing everything now, as the new root path in the dashboard. This is required to secure the password file from access.
