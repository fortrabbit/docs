...
You can extend the concept to have more stages of course (see our [multi staging article](/3.dev/15.learn/03.staging-environments.md)). To add custom rules for a staging environment, like http-auth (see [article](/3.dev/htaccess/3.http-auth.md)), create a matching `.htaccess_staging` file and set the `ENVIRONMENT` accordingly.
