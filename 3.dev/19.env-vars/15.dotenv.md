...
We highly recommend to NOT include a `.env` with your Git repo for obvious security concerns. The .env file can include secrets about your application. You can manually create or copy a `.env` file by SSH/SFTP. This might be automated. In that case, you may remove all custom ENV vars from the dashboard. Please see [ENV var security](/3.dev/19.env-vars/20.security.md).
