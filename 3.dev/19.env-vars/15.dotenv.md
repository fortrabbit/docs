...
Modern PHP frameworks like [Laravel](/2.guides/2.laravel/01.install.md) and [Symfony](/2.guides/7.more/8.symfony.md) …) and some CMS like [Craft CMS](/2.guides/3.craft-cms/01.install.md) use `.env` files for configuration and include a parser already.
...
We highly recommend to NOT include a `.env` with your Git repo for obvious security concerns. The .env file can include secrets about your application. You can manually create or copy a `.env` file by SSH/SFTP. This might be automated. In that case, you may remove all custom ENV vars from the dashboard. Please see [ENV var security](/3.dev/19.env-vars/1.intro.md/11.security.md).
