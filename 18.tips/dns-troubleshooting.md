...
Once a domain is correctly routed, the [TLS certificate](/14.concepts/4.https.md) needs to be installed. This takes some additional time in which requests to the https version of the website will display a certificate warning in the browser. This is actually not a DNS issue, see the [HTTPS troubleshooting guide](/18.tips/https-troubleshooting.md).
...
When visiting your external domain within the [dashboard](/14.concepts/dashboard.md), you'll see the desired and the currently active DNS values for the domain. In general — when not using an external DNS service like [Cloudflare](/16.integrations/cloudflare.md) — those two settings should match.
