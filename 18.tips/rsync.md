---
reviewed:    2023-07-08 15:19:19
title:       rsync
excerpt:     Copy and sync like a boss
...
#      options
#       ─┴─
$ rsync -av ./ {{app-env-name}}@deploy.{{region}}.frbit.com:~/
#           ─┬─ ──────────────────┬────────────────────────
#          source           destination
```
...
$ rsync -av ~/projects/{{app-env-name}}/ ~/projects/{{app-env-name}}.copy/
...
```shell
$ rsync -avn ./ {{app-env-name}}@deploy.{{region}}.frbit.com:~/
...
```shell
rsync -av --include='/{{folder}}/***' --exclude='*' ./ {{app-env-name}}@deploy.{{region}}.frbit.com:~/
```
...
$ rsync -avC --exclude wp-content/themes/your-theme/404.php ./ {{app-env-name}}@deploy.{{region}}.frbit.com:~/
```
...
```shell
# use partial path
$ rsync -avC --exclude themes/your-theme/404.php ./ {{app-env-name}}@deploy.{{region}}.frbit.com:~/

# use smallest possible partial path
$ rsync -avC --exclude 404.php ./ {{app-env-name}}@deploy.{{region}}.frbit.com:~/
```
...
$ rsync -av --exclude-from .rsyncignore ./ {{app-env-name}}@deploy.{{region}}.frbit.com:~/
```
...
```shell
$ rsync -avn --delete ./ {{app-env-name}}@deploy.{{region}}.frbit.com:~/
sending incremental file list
...
$ rsync -av -e 'ssh -i /path/to/your/key' {{app-env-name}}@deploy.{{region}}.frbit.com:~/ ./

# enforce password authentication
$ rsync -av -e 'ssh -o PreferredAuthentications=password' {{app-env-name}}@deploy.{{region}}.frbit.com:~/ ./
```
