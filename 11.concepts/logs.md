...
You see a [500 error](/14.tips/http-errors/500.md) and you need to know why. PHP error logs contain information about issues that occur while executing PHP code. Use these logs to identify bugs in the code or configuration. PHP error logs typically include information such as the time and date of the error, the type of error, and the file and line number where the error occurred.
...
Most CMS and frameworks also offer options to send these logs to the standard locations. With the [software preset](/11.concepts/software-presets.md) the setting is getting pre-populated, if possible, by ENV var.
