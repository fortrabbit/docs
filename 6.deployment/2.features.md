...
1. The 'fortrabbit GitHub app' receives Git changes
2. The build step pipeline on fortrabbit runs the deployment
...
Your `git push` updates the Git remote and triggers the build of a new release package. This new release package will then be distributed to the fortrabbit hosting platform. Files will then be replaced by the ones contained in the release package.
