...
Make sure to be able to access the fortrabbit MySQL database, see the [MySQL access article](2.access.md).
...
```shell
mysql -h127.0.0.1 -P13306 -u{{app-env-slug}} -p {{app-env-slug}} < dump.sql
```
...
$ mysql --local-infile=1 -h127.0.0.1 -P13306 -u{{app-env-slug}} -p {{app-env-slug}}

# on the mysql shell
$ mysql> LOAD DATA LOCAL INFILE '/path/to/tablename.sql' INTO TABLE tablename;
```

**Note**: You will be asked to enter your App's database password. [Look it up in the Dashboard](https://dashboard.fortrabbit.com/apps/{{app-env-slug}}#mysql).
