...
The idea is to create another repo on the app environment, which has the same upstream as your local repo: both the local and the app environment repos are sharing the same upstream. So the first step is to create this repo on the app environment, then create a new branch there, which is where we put any new content created on the app environment. (We create this new branch because we do not want to push changes to the master branch on the deploy service, because this would trigger a deployment.) Then, any changes we make to content on the app environment can then be pulled into your local repo, where it can be merged.
...
1. Set up an SSH key with SSH keygen (no passphrase), see our [SSH keys setup guide](/ssh-keys#generate-a-ssh-key-pair-aka-ssh-key-)
2. Copy the content of the public key (likely `/srv/app/{{app-env-name}}/htdocs/.ssh/id_ed25519_fortrabbit.pub`) to a buffer
...
Host deploy.{{region}}.frbit.com
    IdentityFile /srv/app/{{app-env-name}}/htdocs/.ssh/id_ed25519_fortrabbit
```
...
5. Add the fortrabbit deploy service as a remote (`git remote add fortrabbit {{ssh-user}}@deploy.{{region}}.frbit.com:{{app-env-name}}.git`)
...
See the [Statamic Git guide](https://statamic.dev/git-automation#remote-setup) and the [fortrabbit git remote help](/git-deployment#adding-fortrabbit-as-a-remote) as reference.
