...
2. Copy the content of the public key (likely `/srv/app/{{app-env-id}}/htdocs/.ssh/id_ed25519_fortrabbit.pub`) to a buffer
...
Host deploy.{{region}}.frbit.com
    IdentityFile /srv/app/{{app-env-id}}/htdocs/.ssh/id_ed25519_fortrabbit
```

## 5 - Init Git on the app environment

<!-- TODO: Rewrite for GitHub deployment -->
...
5. Add the fortrabbit deploy service as a remote (`git remote add fortrabbit {{ssh-user}}@deploy.{{region}}.frbit.com:{{app-env-id}}.git`)
