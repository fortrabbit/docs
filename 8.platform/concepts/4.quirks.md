...
Only the branches `main`, `master` and the `{{app-name}}` will be deployed to the webspace. More [here](2.git-deployment.md#toc-the-branch-name-counts).

### Release package limit

To keep deployment fast for everyone the size of the release package is limited to 200 MB. [Read on](8.platform/concepts/3.environment/git.md#toc-release-package-limit).
