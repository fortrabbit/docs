...
naviTitle:        Updating
excerpt:          Opinionated guide to update Craft
...
The `config/general.php` file we use in [our setup guide](/craft-setup#configuration-settings) sets `allowUpdates` to `false`: this prevents plugin and Craft core updates from the Craft Control Panel in the `production` environment (fortrabbit App).
...
As of Craft 3.5 all essential settings will be stored in a `project.yaml` file. See also [here](/craft-tune#using-project-config).
