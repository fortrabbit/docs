...
reviewer: fl
title: The domain
description: 'Every fortrabbit environment carries a test domain, and external domains registered elsewhere can be routed to it as well.'
naviTitle: Domain
...
- [Domain provider integration](/4.integrations/02.dns/01.intro.md) - combine domain provider with fortrabbit
- [External domains intro](/1.platform/13.dns/02.external-domains.md) - options and limits
- [Point an external domain](/1.platform/13.dns/03.external-domain-setup.md) - step by step instructions
...
A root path is a folder in the [file system](/1.platform/05.code-access/05.directories.md) of the [environment](/1.platform/10.objects/02.environment.md) where a domain will end. The root path is saved with the environment for all its domains. That means no interface is offered to configure individual root paths per domain. You can, however, configure custom routing of domains in your code and config, either by `.htaccess` rules or by some routing options with your software.

### Domain forwarding

The dashboard allows you to forward domains with the same environment. Usually you will want to forward all domains to your main domain. See the [www domain forwarding article](/1.platform/13.dns/04.domain-forwarding-service.md).
