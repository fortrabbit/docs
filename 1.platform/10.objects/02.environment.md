...
description: 'An environment is one running version of a project, with its own domains, settings, components and bill, inside an app.'
...
head:
  meta:
    - name: keywords
      content: 'environment, app environment, hosting, git branch, staging, deployment, fortrabbit'
topics:
  - dashboard
  - scaling
  - env-vars
...
- An environment is a sub object of an [app](/1.platform/10.objects/01.app.md)
- Each app needs to have at least one environment
- Environments are managed in the dashboard
- Each environment is a separated hosting setup, with [individual settings](/1.platform/07.settings/index.md)
...
We usually use 'environment', in some contexts maybe 'app environment' since this explicitly names the fortrabbit object and can not be confused with a development environment. In the dashboard, the environment title will be constructed like so: `{app name} / {environment id}`.

## Environment maps to git branch

While the Git repo is connected with the [app](/1.platform/10.objects/01.app.md), the environment can be mapped to a git branch. [Git deployment](/1.platform/06.deployment/01.intro.md) is optional but blends in nicely.
...
The environment has a lot of settings exposed in the fortrabbit dashboard. See the [hosting settings topic](/1.platform/07.settings/index.md).
