---
reviewed: 2026-03-22 11:00:19
draft: true
title: Key-value store
description: 'The key-value store is a Valkey instance, Redis-compatible, usable as a cache, a session store or a queue broker next to an app.'
navigation.excerpt: Database, cache, queue message broker
lead: In-memory data store based on Valkey.
figure:
  emoji: 🧠
  text: Very fast memory.
  color: rgb(39, 105, 95)
  textColor: rgb(255, 190, 240)
draft: true
subject: key-value-store
topics:
  - scaling
  - performance
---
...
:BlockLink{title="Book key-value store" path="/environments/{{app-env-id}}/components#key-value-store"}
...
The key-value store is implemented as an optional component on fortrabbit with various plans differing the available memory. It can be booked and scaled per [environment](/1.platform/10.objects/02.environment.md). To book and scale the key-value store, navigate to the dashboard and select your environment. Find the components section. Choose the key-value store and select the desired plan based on your memory requirements. Confirm your selection to book the key-value store.
...
Valkey is supported by many PHP frameworks and CMS's out of the box, it's popular among [Laravel](/2.guides/2.laravel/01.install.md) developers. For specific integrations check out the install guides and find out how to use it with your favorite framework or CMS.
...
- [environments](/1.platform/10.objects/02.environment.md)
