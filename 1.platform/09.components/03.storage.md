...
# Show 10 biggest directories here
...
- `df` reports actual disk free space and is more accure for how files are stored on disk. On fortrabbit run `df -h` and look for the `csi-vol-` entry to see the real disk usage, or you can just look at the dashboard metrics to see the same value.
