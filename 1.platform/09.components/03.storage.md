...
reviewer: fl
title: Storage
description: 'The storage component is persistent web space for deployed code and runtime files, reachable over SSH and SFTP and required per environment.'
...
The storage is integrated as a persistent file system. You have direct [code access](/1.platform/05.code-access/01.intro.md) by [SSH](/1.platform/05.code-access/03.ssh.md) and [SFTP](/1.platform/05.code-access/04.sftp.md). The storage contains the deployed code, as well as runtime data, such as uploads and temporary files. See the [directory structure article](/1.platform/05.code-access/05.directories.md) to learn about pre-configured folders. The dashboard shows metrics about the storage size.
...
- [Connect via SSH](/1.platform/05.code-access/03.ssh.md) or [SFTP](/1.platform/05.code-access/04.sftp.md)
...
- `du` counts file sizes and sums them together to give you the size of all files in a directory. This is an estimate of disk usage and can differ from actual disk usage for many reasons. Great for finding large files to delete.

- `df` reports actual disk free space and is more accurate for how files are stored on disk. On fortrabbit run `df -h .` to see the real disk usage, or you can just look at the dashboard metrics to see the same value.
...
Web storage backups are available through the additional [backup component](/1.platform/09.components/05.backups.md).
