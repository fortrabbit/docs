...
description: 'SSH, SFTP or database login refused? Most causes sit on the local machine. Test SSH first, then work through the usual suspects.'
...
head:
  meta:
    - name: 'keywords'
      content: 'ssh key, ssh, git, git-deployment, username, password, access, authentication, fortrabbit'
topics:
  - ssh
  - sftp
  - troubleshooting
...
- Have you set up SSH keys and uploaded the public part to fortrabbit?
- Have you been able to [access code](/1.platform/05.code-access/01.intro.md) before?
...
Open the files `~/.ssh/config` and `~/.ssh/known_hosts` and see if there is anything unusual. Delete all entries related to fortrabbit services to reset your local configuration. See our [SSH configuration guide](/1.platform/05.code-access/03.ssh.md) for details on proper setup.

## Access error for recently created environment

If an [environment](/1.platform/10.objects/02.environment.md) was recently created, please wait about 5 minutes for it to become ready. Trying to connect before it is ready produces an error. If more than 10 minutes have passed and you still get a similar error (despite successful authentication), then please contact us.
...
The first time you connect to fortrabbit services via SSH, there will be a warning about connection to a new SSH server. Just type yes, which will record the fingerprint of the fortrabbit deploy service into the `~/.ssh/known_hosts` file. Say yes here:
...
If this fingerprint changes in the future, then you will see an error about that and will have to take further action depending on the situation.
