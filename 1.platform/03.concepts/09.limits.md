...
lead: Quirks, limits, restrictions, permissions — aren't there always some? Heads up so it doesn't cost you hours of researching in the wrong direction.
siblings: Concepts
---
...
**Recommended**: to use a "transactional mail service", those are built to do the automated mailings — see [transactional mail](/1.platform/15.integrations/13.transactional-mail-providers.md) for more details. Such providers are usually easy integrated via an API and there are plugins for CMS and frameworks.
...
To use this option anyhow, it's required to manually open the SMTP ports (25, 465, 587) with the [outgoing firewall rules](/1.platform/06.settings/21.firewall-outgoing.md). Some mail providers may require to provide an IP address from which mails should accepted. An outgoing IP can however not be guaranteed to be persistent from our side.

### No sendmail

**Not available**: It's not possible to use sendmail — the Mail Transfer Agent — on fortrabbit. In PHP Sendmail is usually used with the `mail()` function. Back in the good ol' days, this would simply call the shell command `sendmail` from the shell to send a mail directly from the web server to the receiving mail server. This is a bad practice nowadays: your web server can send mails, but not receive any - it is not a mail server. So it is not distinguishable from any home PC part of a bot net.
