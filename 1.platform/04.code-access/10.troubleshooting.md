...
Open the files `~/.ssh/config` and `~/.ssh/known_hosts` and see if there is anything unusual. Delete all entries related to fortrabbit services to reset your local configuration. See our [SSH configuration guide](/1.platform/04.code-access/03.ssh.md) for details on proper setup.
