...
description: 'Database connection problems on fortrabbit, from the missing SSH tunnel to max_user_connections, with the fix for each of them.'
...
The most common misunderstanding when trying to connect from a local machine is that people overlook the need to open an SSH tunnel before connecting to the database. Graphical MySQL clients support this connection method out of the box.

You'll need to **enter both your SSH and MySQL access details**. Within the fortrabbit dashboard under your App > Access, there is a small link labeled: "Show SSH tunnel info" which will reveal everything you'll need to enter in a MySQL client to connect to the remote database.
...
You can't INSERT to the database anymore? You'll likely see something like this:

```sql
GRANT SELECT, UPDATE, DELETE, DROP, ALTER ON `{{app-env-id}}`.* TO `{{app-env-id}}`@`%`
```

**The database of the environment is full.** It has exceeded the available storage size and [autoscaling](/1.platform/12.billing/11.autoscaling.md) is not enabled. In many cases, the database grows with passive usage. What you can do:

- Upgrade the plan or enable autoscaling in the dashboard
- Query large tables, see [below](#query-large-tables)
