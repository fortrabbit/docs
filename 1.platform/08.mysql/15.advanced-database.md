...
description: 'Advanced MySQL on fortrabbit: time zones, password resets, storage engine limits, GTID flags for imports and cross-environment access.'
naviTitle: Advanced topics
navigation.excerpt: Limits, procedures, time zones, triggers …
lead: Advanced MySQL configuration — managing time zones, resetting passwords, understanding storage engine limits, GTID flags for imports, and database access across environments.
figure:
  emoji: ⚙️
...
head:
  meta:
    - name: keywords
      content: 'mysql, database, time zone, storage engine, procedures, triggers, innodb, gtid, limits, fortrabbit'
topics:
  - mysql
  - performance
---
...
The `--set-gtid-purged=OFF` option prevents GTID-related permission errors during import. See [database import via terminal](/1.platform/08.mysql/02.export-import/08.database-up-terminal.md) for full context. This is required when:
...
- Unless you are using [env vars](/3.dev/19.env-vars/01.intro.md): You'll need to change the password in your code configuration files
...
fortrabbit only supports the InnoDB storage engine. If your source database uses MyISAM or another engine, convert all affected tables **before** importing — otherwise the import will fail. See [database import](/1.platform/08.mysql/02.export-import/08.database-up-terminal.md) for details on uploading your database.

To check which tables need converting, run this on your source database:
...
### Option 1: Convert in the source database

Run this for each affected table before creating the dump:

```sql
ALTER TABLE table_name ENGINE=InnoDB;
```

### Option 2: Edit the SQL dump file

After exporting, replace all occurrences in the `.sql` file — with a text editor or on the command line:

```bash
sed -i 's/ENGINE=MyISAM/ENGINE=InnoDB/g' your-dump.sql
```
