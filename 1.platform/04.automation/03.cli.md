...
lead: Use the fortrabbit CLI to manage apps, environments and deployments from a terminal, a script or a CI job instead of the dashboard.
links:
...
The CLI is a single binary named `frbit`. It is a client of the [public API](/1.platform/04.automation/05.api.md) and it authenticates with your personal API token.

## Use cases

- A person works from a terminal, or a script runs unattended
- A CI pipeline deploys, restarts, or sets environment variables
- An agent with shell access needs what the [MCP server](/1.platform/04.automation/02.mcp.md) does not expose
- One command sets up the MCP server and the skills: `frbit setup agent`
...
The source code, the release archives, and the full command reference live in the [frbit-cli repository](https://github.com/fortrabbit/frbit-cli) on GitHub.
