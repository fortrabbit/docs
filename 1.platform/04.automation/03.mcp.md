...
The command opens a browser page that names the requesting client and shows the exact address the approval is sent back to. Check both before approving. Approval issues an access token that expires after one hour and a refresh token that lasts 30 days, both bound to that one agent.
