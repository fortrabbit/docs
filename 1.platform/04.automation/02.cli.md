...
lead: The fortrabbit CLI is a single binary named frbit that talks to the public API, so apps, environments and deployments can be managed from a terminal, a script or a CI job instead of the dashboard.
links:
  - title: Public API
...
It ships as a self-contained binary with no runtime dependency, so it also fits into CI pipelines and containers. The source code, the release archives and the full command reference live in the [frbit-cli repository](https://github.com/fortrabbit/frbit-cli) on GitHub.

## Install

The install script is the shortest path on macOS and Linux. Homebrew and npm suit machines that already manage tooling that way, and the release archives cover everything else, Windows included.
...
The CLI signs in once and keeps the token in the operating-system credential store. Automated jobs skip that and pass the token per command instead.

Create a personal API token under `API tokens` in the account menu of the dashboard, then run:
...
The CLI also wires fortrabbit into a coding agent, so the [MCP server](/1.platform/04.automation/03.mcp.md) and the [agent skills](/1.platform/04.automation/04.agent-skills.md) do not have to be registered by hand:
