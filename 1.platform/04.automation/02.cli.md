...
Reach for the CLI when a person or a script drives the work. For an AI agent, the [MCP server](/1.platform/04.automation/03.mcp.md) is the better fit, because the agent discovers the available actions instead of being told the command names.

The CLI ships as a self-contained binary with no runtime dependency, so it also fits into CI pipelines and containers. The source code, the release archives and the full command reference live in the [frbit-cli repository](https://github.com/fortrabbit/frbit-cli) on GitHub.
