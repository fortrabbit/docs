...
reviewed: 2026-08-22
title: MCP server
description: 'The fortrabbit MCP server connects an AI agent to apps and environments. Authorization runs in the browser, so no token is pasted anywhere.'
naviTitle: MCP
navigation.excerpt: Connect an AI agent to the platform
ai: co-authored
figure:
  emoji: 🧩
  text: AI agents plugin.
  color: rgb(45, 35, 75)
...
- `https://mcp.fortrabbit.com/mcp` &larr; remote endpoint

Requests pass the same authorization as the [public API](/1.platform/04.automation/05.api.md), so an agent never sees more than the person who authorized it.
...
A client without OAuth support takes a personal [API token](/1.platform/04.automation/05.api.md) as a bearer credential instead:
