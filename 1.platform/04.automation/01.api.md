...
lead: The fortrabbit public API is a REST interface to apps, environments, deployments and domains. It authenticates with a personal API token and is the foundation the CLI builds on.
links:
  - title: CLI
...
The API is like the whole platform in beta. Breaking changes are possible while that lasts, so pin nothing that cannot be adjusted. The `/v1` prefix marks the shape of the API.
