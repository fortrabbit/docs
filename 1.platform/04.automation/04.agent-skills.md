...
Removing the skills leaves an MCP connection untouched. That comes off separately, described under uninstall on the [MCP server](/1.platform/04.automation/03.mcp.md) page.
...
The `fortrabbit-api-access` skill also walks through it when asked. Either way, the command only writes configuration — the browser approval follows on the first connection, which the [MCP server](/1.platform/04.automation/03.mcp.md) page covers along with the tools behind it.
