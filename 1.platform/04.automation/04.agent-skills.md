...
Codex publishes the same kind of client metadata document, so its command has the same shape:

```shell
codex mcp add fortrabbit --url https://api.fortrabbit.com/mcp
```

On Codex older than 0.147 the document is missing, so the client ID goes in by hand:
