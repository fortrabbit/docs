...
It can not be guaranteed that the backup restoration will leave the environment in a working state. It's possible that the environment returns an error after the restoration. In many cases that's a [500 error](/3.dev/22.http-errors/03.504.md), which is easy to debug, by looking at the logs.
