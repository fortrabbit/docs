...
All logs are available through the [fortrabbit dashboard](/1.platform/02.concepts/30.dashboard.md), individual per [app environment](/1.platform/10.objects/02.app-environment.md). A live log stream as well as historical logs are available for `stdout` and `stderr`.
...
You are using [Git deployment](/3.dev/03.deployment/01.intro.md) with some [build steps](/3.dev/03.deployment/3.build-commands.md) but for some reason nothing get's deployed or you are expecting a different result. Use the deployment logs to see what's happening during deployment.
...
Your [worker or cron job](/1.platform/09.components/7.jobs.md) is crashing. You want to know why. Use the logs to see what your workers have been doing.

## Log files in your CMS or framework

Mind that your CMS or framework might pipe logs to a different location, other than `stderr` and `stdout`. If they do, you will not see the error output in the fortrabbit dashboard. You can still access the logs by [SSH](/3.dev/01.code-access/3.ssh.md) or [SFTP](/3.dev/01.code-access/4.sftp.md), please see your CMS / framework docs where the logs are stored.
