...
reviewer: fl
title: Deploy hooks
description: 'A deploy hook is a URL that triggers a deployment on POST — the way a headless CMS or a cron job can publish without a git push.'
...
head:
  meta:
    - name: keywords
      content: 'deploy hook, webhook, deployment trigger, GitHub Actions, GitLab CI, fortrabbit'
topics:
  - deployment
  - automation
---
...
NOTE that this is for demonstrational purposes only. Don't take that example literal. [GitHub Actions](/4.integrations/03.git-providers/02.github.md) is one kind of integration. Read on for [alternatives](#alternatives).

## Alternatives

The deploy hook is a specific solution for specific workflows.

For most day-to-day usage, the [GitHub integration](/1.platform/06.deployment/02.github-app.md) with automatic `git push` to deploy is the most convenient way to deploy.
