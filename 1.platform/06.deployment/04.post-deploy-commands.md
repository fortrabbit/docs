...
description: 'Post deploy commands run once the build is in place, which is where database migrations and cache clearing belong.'
naviTitle: Post deploy commands
navigation.excerpt: Run migrations after deployment
figure:
  emoji: ✅
  text: Execute tasks after deployment.
  color: rgb(34, 150, 60)
...
head:
  meta:
    - name: keywords
      content: 'post deploy commands, deployment tasks, migrations, cache clearing, fortrabbit'
topics:
  - deployment
  - php
---
...
You have followed the [deployment section](/1.platform/06.deployment/index.md) and are ready to deploy code via git using the [GitHub integration](/1.platform/06.deployment/02.github-app.md).

## Architecture

Post deploy commands are similar to [build commands](/1.platform/06.deployment/03.build-commands.md), but they run after the build files have been copied into the live environment. Both build commands and post deploy commands are run in a separate deployment container.
...
:BlockLink{title="Edit post deploy commands for {{app-env-id}}" path="/environments/{{app-env-id}}/deployment/post-deploy-commands"}

## Preset via software templates

Many [software presets](/1.platform/03.concepts/05.software-templates.md), such as those for Laravel, Symfony and Craft CMS, already include pre-configured post deploy commands.
...
- **Cache management:** Consider the impact before clearing all caches every time, especially in production. Cache clearing consumes resources, and rebuilding caches can significantly impact performance.
