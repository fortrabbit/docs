---
reviewed: 2026-05-07
title: 'New fortrabbit platform: features and changes'
description: 'What changed on the new fortrabbit platform: GitHub deployment, persistent storage, component pricing, free collaboration, and what was dropped.'
naviTitle: What's new
...
lead: The new fortrabbit platform introduces GitHub deployment, persistent storage, component-based pricing, and free collaboration — here is a full breakdown of what changed and what was deprecated.
head:
  meta:
    - name: keywords
      content: 'fortrabbit new platform, platform changes, GitHub deployment, component pricing, deprecations'
nextNav: true
topics:
  - migration
  - updates
...
**fortrabbit no longer provides Git repo hosting.** Connect your GitHub (other providers like GitLab and CodeBerg are planned) with the fortrabbit GitHub app to enable `git push` deployment instead. See the [GitHub integration](/1.platform/06.deployment/02.github-app.md).
...
Mind that using our [Git deployment](/1.platform/06.deployment/01.intro.md) is not a requirement, it's optional. Direct [SSH/SFTP access](/1.platform/05.code-access/01.intro.md) is always possible. This already allows integrations with other Git providers today, see the [Git providers integration section](/4.integrations/03.git-providers/).

### Build commands

The deployment pipeline is more configurable. Node.js is available during the deployment to create JavaScript bundles. See the new [build commands article](/1.platform/06.deployment/03.build-commands.md).
...
Billing concepts mostly stay the same. There still is [pro-rated post-paid billing](/1.platform/12.billing/02.postpaid.md). See [price comparison](/1.platform/01.new/7.price-comparison.md) for more details. Note that prices can still change during BETA.
...
- Craft Copy is a compatibility layer, parts are not required any more:
  - delete config files -> [replace patterns](/1.platform/06.deployment/15.replace-patterns.md)
  - auto-migrate -> [post deploy commands](/1.platform/06.deployment/04.post-deploy-commands.md)
...
- Other parts, like `db/down`, might be replaced by general tooling:
  - [fortrabbit agent skills](/1.platform/04.automation/04.agent-skills.md)
  - fortrabbit CLI
