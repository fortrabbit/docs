...
If you did not choose Symfony when creating the app environment in the dashboard at first, please set the following: Go to the Dashboard and [set the root path](/app#root-path) of your App's domains to 'public'.

<div markdown="1" data-user="known">
[Change the root path for App URL of App: **{{app-env-name}}**](https://dashboard.fortrabbit.com/apps/{{app-env-name}}/rootpath)
</div>
...
<div markdown="1" data-user="known">
[Go to ENV vars for the App: **{{app-env-name}}**](https://dashboard.fortrabbit.com/apps/{{app-env-name}}/vars)
</div>
...
**Got an error?** Please see the [access troubleshooting](/access-methods#troubleshooting). **Did it work?** Cool! This first push can take a bit, since all the Composer packages need to be installed. When the first push is done you can visit your App URL in the browser and see the Symfony welcome screen:
...
The `DATABASE_URL` ENV var stores all DB access information already (see [above](#env-vars)). You just need to use it in `doctrine.yaml` like so:  
...
$ rsync -av ./public/build/ {{app-env-name}}@deploy.{{region}}.frbit.com:~/public/build/
```
...
In Universal Stack, you can access all log files your App writes on the file system. If you want to use live logging in [Universal Stack](logging-uni#live-log-access) or in [Professional Stack](logging-pro), then you should configure Symfony to use `error_log`. After having installed monolog bundle (`composer require symfony/monolog-bundle`), modify the `config/packages/prod/monolog.yml` file:
...
You can not use [sendmail](quirks#mailing) on fortrabbit but you can use the `Swiftmailer`.
