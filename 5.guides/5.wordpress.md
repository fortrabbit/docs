...
<!-- TODO: Review and make shorter  -->

## Get ready

We assume you've already created an [App](app) and chose WordPress in the [Software Preset](/12.platform/4.details/software-presets.md). If not: You can do so in the [fortrabbit Dashboard](/dashboard). Following the fastest way to start with a fresh installation. Please scroll below for [migrating an existing WordPress](#advanced-setup).
...
Now copy the **contents** of the local `wordpress` folder (not the folder itself) via SFTP to the `htdocs` folder of your App. The `htdocs` folder is the one you are automatically in after logging in via SFTP. The SFTP access for your App **{{app-env-name}}** is:

* **Server**: `deploy.{{region}}.frbit.com`
* **User name**: `{{ssh-user}}`
* **Password**: `{{ssh-password}}`

The upload will take around 7 minutes, depending on your internet speed. A quicker method is shown [below](#installing-wordpress-with-ssh).

### 3. Run the installer

When the upload is finished, visit [{{app-env-name}}.frb.io](https://{{app-env-name}}.frb.io) in your browser and commence with the guided web installation to finish the setup. You will need MySQL access, which is:

* **Database name**: `{{app-env-name}}`
* **User name**: `{{app-env-name}}`
* **Password**: [Look up in the Dashboard](https://dashboard.fortrabbit.com/apps/{{app-env-name}}#mysql)
* **Database host**: `{{app-env-name}}.mysql.{{region}}.frbit.com` _< not localhost_
...
* [{{app-env-name}}.frb.io](https://{{app-env-name}}.frb.io) _< WordPress installation_
* [{{app-env-name}}.frb.io/wp-admin](https://{{app-env-name}}.frb.io/wp-admin) _< WordPress admin_
...
Read on in the [MySQL Article: Export & import > Using MySQL Workbench (GUI)](mysql-uni#using-mysql-workbench-gui-).

#### Migrating the database in the terminal

Read on in the [MySQL Article: Export & import > Using the terminal](mysql-uni#using-the-terminal).
...
One strategy to simplify your life is to use SFTP synchronization. Please see the dedicated [section in the SFTP article](sftp#file-synchronization) on that.
...
You cannot use [sendmail](quirks#mailing) on fortrabbit but you can use a SMTP plugin like [WP SMTP](http://wordpress.org/plugins/wp-smtp/) or [MAIL SMTP](http://wordpress.org/plugins/wp-mail-smtp/) to enable SMTP support for the `wp_mail()` function.
...
1. Connect to the remote MySQL database from local (see [here how](mysql.md#access-mysql-database-from-local))
...
2. You want to run multiple WordPress sites in one App. [Please don't.](/app#one-website-per-app)
...
Usually we advise using one App per website (see [here](/app#one-website-per-app)). But you can use the website "network" feature (previously multi-site) with one WordPress installation here as well. You might point different domains to one App.
