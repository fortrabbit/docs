...
We assume you've already created an [app](/10.objects/1.app.md) and chose WordPress in the [software preset](/12.platform/4.details/software-presets.md). If not: You can do so in the [fortrabbit dashboard](/11.concepts/dashboard.md). Following the fastest way to start with a fresh installation.

## 1. Download WordPress

Start by downloading the [latest WordPress archive](https://wordpress.org/latest.zip) from the WordPress website and unpack it to your local computer. It will extract into the folder `wordpress`.

## 2. Upload WordPress files

Now copy the **contents** of the local `wordpress` folder (not the folder itself) via SFTP to the `htdocs` folder of your App. The `htdocs` folder is the one you are automatically in after logging in via SFTP. The SFTP access for your {{app-env-name}} is. The upload will take a couple of minutes minutes, depending on your internet speed.

### 3. Run the installer

When the upload is finished, visit [{{app-env-name}}.frb.io](https://{{app-env-name}}.frb.io) in your browser and commence with the guided web installation to finish the setup. See the dashboard to look up your database credentials. The installer will also ask you for your WordPress site's Site title, User name, Password, Your email and so on. After the guided web setup is done, you will be automatically redirected to the WordPress admin login form. Use the previously chosen Username and Password to login.
...
WordPress itself has not arrived in the new PHP age in terms of using the latest technologies and paradigms. So — we do not recommend using the standard WordPress with Git and Composer. But there is a super-cool WordPress boilerplate called [Bedrock](https://roots.io/bedrock/).
...
Your environment URL `{{app-env-name}}.frb.io` is the first address your WordPress can be reached. Later on, when you go live, you will add your own custom external domains. Here is the basic setup:
...
1. For the **database**, check our [MySQL article for import/export](/mysql).
