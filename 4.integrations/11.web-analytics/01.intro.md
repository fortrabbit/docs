...
description: 'Web analytics for a fortrabbit app: what visitor tracking shows, and how privacy-first tools compare with Google Analytics under GDPR.'
naviTitle: Intro
...
head:
  meta:
    - name: 'keywords'
      content: 'web analytics, tracking, privacy, gdpr, fortrabbit'
topics:
  - monitoring
  - frontend
---
...
fortrabbit provides server-side analytics called [metrics](/1.platform/07.settings/30.metrics.md) too. What's the difference?

Server side analytics can be an alternative to client-side JavaScript tracking. Instead of sending data from the browser, analytics data is generated and collected on the web server. Server-side analytics work well in combination with client-side solutions to get a complete picture of user behavior and application performance. In many cases server-side analytics can provide enough insights. Note that the data aggregation and type of data are different and cannot be directly compared.

Before modern browser based web analytics became popular, tools like [Webalizer](https://webalizer.net/), [AWStats](https://www.awstats.org/), and [Analog](https://analog.gsp.com/) analyzed web server log files to generate statistics. Those tools were basically a web UI for the server log files. It's rather sad that these tools are rarely used today.
