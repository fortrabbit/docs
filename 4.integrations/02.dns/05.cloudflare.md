...
reviewer: fl
title: Cloudflare
description: 'Cloudflare works at the DNS level as a CDN, a DDoS shield and a source of TLS certificates. What to enable in front of a fortrabbit app.'
navigation.excerpt: Domains, DNS and DDOS protection
figure:
  emoji: ☁️
  text: Domains, CDN, DNS and DDOS protection.
  color: rgb(200, 120, 40)
  textColor: rgb(254, 243, 199)
head:
  meta:
    - name: 'keywords'
      content: 'cloudflare, dns, fortrabbit, cdn, ssl, ddos, proxy'
topics:
  - dns
  - https
  - performance
...
3. Route the domain using our standard settings with your old DNS provider
...
Each domain on Cloudflare comes with a bunch of settings. One is called "Crypto" and is about SSL. Your goal is that the whole communication between you and your users is encrypted. Now, Cloudflare, as described above, is a proxy between your user and fortrabbit. The first leg from the user to Cloudflare is secured by default, but what about the next one, between Cloudflare and fortrabbit?
...
The test domain has HTTPS, so you can and should set SSL to **full** to ensure end-to-end encryption. The default **flexible** setting is not enough (as we think). "Full (strict)" mode will not work but is not necessary.

## Advanced topics

### Cloudflare DNS and fortrabbit Dashboard Status

Cloudflare is a black box, DNS-wise. When you visit a domain in the fortrabbit Dashboard that is routed via Cloudflare an accurate response is not possible. The Dashboard is aware of many Cloudflare IPs, so it will likely guess that the domain is routed via Cloudflare. But you might still see an error, even if the domain is in fact routed correctly.

### Cloudflare SSL VS Let's Encrypt

Many fortrabbit clients have used Cloudflare to get SSL for their own custom domain, without the need to book and set up a custom cert here. Now, fortrabbit also offers free SSL certificates via Let's Encrypt (free and zero-config). So if that is your aim, you might not need Cloudflare.
...
- CNAME Flattening for [apex domains](/1.platform/13.dns/07.apex-domains.md)
- Domain analytics

### Prevent direct access

You may want to ensure all traffic is routed through Cloudflare. This is important to secure 360 DDoS protection. You'll basically disable the App URL to be accessible. To block direct access and whitelist only Cloudflare's IPs, extend your `.htaccess` file with the following rules. Cloudflare occasionally changes these ranges, so verify them against the canonical list at [cloudflare.com/ips](https://www.cloudflare.com/ips/):
