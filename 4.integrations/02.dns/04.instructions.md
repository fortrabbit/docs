...
reviewed: 2026-07-09
reviewer: fl
title: Domain provider instructions
description: 'Point an external domain at fortrabbit: CNAME and A record instructions for the common domain providers, step by step for each one.'
naviTitle: Instructions
navigation.excerpt: How to point your external domain to fortrabbit
lead: How to configure your domain provider to point your external domain to fortrabbit.
head:
  meta:
    - name: keywords
      content: DNS, CNAME, A record, domain configuration, fortrabbit
topics:
  - dns
---
...
You should have a domain created with the fortrabbit dashboard. See the [domain setup guide](/1.platform/13.dns/03.external-domain-setup.md). You should also have a domain registered somewhere. See [domain provider intro](/4.integrations/02.dns/01.intro.md).

## Instructions for domain providers

Below are links to instructions for setting CNAME and A records for specific domain providers. These settings are typically configured through the provider's control panel.
...
- [Porkbun: CNAME instructions](https://kb.porkbun.com/article/231-how-to-add-dns-records-on-porkbun) General DNS instructions
- [Porkbun: A record instructions](https://kb.porkbun.com/article/54-pointing-your-domain-to-hosting-with-a-records) forwarding apex domains

### Cloudflare Domains

- [Cloudflare: Adding a DNS record](https://support.cloudflare.com/hc/en-us/articles/360019093151-Adding-DNS-records) — see also [Cloudflare intro](/4.integrations/02.dns/05.cloudflare.md)
