...
description: 'fortrabbit runs no hosted phpMyAdmin. A native database client connects over an SSH tunnel — faster, encrypted, and usable for local work too.'
naviTitle: Intro
ai: co-authored
reviewer: fl
...
head:
  meta:
    - name: 'keywords'
      content: 'database client, mysql, gui, ssh tunnel, phpmyadmin, fortrabbit'
topics:
  - mysql
---
...
A native database client offers four distinct advantages over web-based solutions: secure SSH-tunneled connections that encrypt credentials, fast performance on large datasets without browser overhead, intelligent query tools with auto-completion and history, and unified access to multiple databases from a single application.
...
Popular database clients offer varying support across operating systems, with macOS having the most options, Windows and Linux sharing many cross-platform tools, and specialized tools serving specific platform ecosystems.
